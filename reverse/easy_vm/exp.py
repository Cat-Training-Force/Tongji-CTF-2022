...
def dasm(c,v,so:z3.Solver):
    ...
    while ip<len(c):
        ...
        if op==0:
            ...
            state[sp-1]=state[sp]+state[sp-1]
            # state[sp-1]&=0xffffffff
            sp-=1
        elif op==2:
            state[sp-1]=state[sp]-state[sp-1]
            # state[sp-1]&=0xffffffff
            ...
            sp+=1
            state[sp]=u32(c[ip+1:ip+5])
        elif op==6:
            sp+=1
            state[sp]=state[sp-1]
        elif op==5:
            sp+=1
            state[sp]=v[u32(c[ip+1:ip+5])]
        elif op==8:
            so.add(state[sp]==state[sp-1])
            sp-=1
        if op==5 or op==7:
            ip+=5
        else:
...
v=[z3.BitVec('v%d'%i,32) for i in range(8)]
s=z3.Solver()
for i in v:
    s.add(i&0xff>=ord('0'))
    s.add(i&0xff<=ord('z'))
    s.add((i>>8)&0xff>=ord('0'))
    s.add((i>>8)&0xff<=ord('z'))
    s.add((i>>16)&0xff>=ord('0'))
    s.add((i>>16)&0xff<=ord('z'))
    s.add((i>>24)&0xff>=ord('0'))
    s.add((i>>24)&0xff<=ord('z'))
dasm(code,v,s)
print(s)
flag=b''
if s.check()==z3.sat:
